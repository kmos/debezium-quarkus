...
- Native build
- developer experience that follows the interfaces provided by the `debezium-quarkus-engine`
...
```xml
        <dependency>
            ...
            <artifactId>debezium-quarkus-engine-spi</artifactId>
        </dependency>
        <dependency>
            <groupId>io.debezium.quarkus</groupId>
...
```xml
        ...
        <dependency>
            <groupId>io.debezium.quarkus</groupId>
            <artifactId>debezium-quarkus-engine-deployment</artifactId>
        </dependency>
        <dependency>
            <groupId>io.quarkus</groupId>
...
Even if it isn't strictly necessary to implement the `QuarkusEngineProcessor`, it helps to adhere to the expected beans that the `debezium-quarkus-engine` needs to work.


### Testing the deployment module

To test that the deployment module (and the runtime) are correct, you can use the `debezium-quarkus-testsuite` that provides a suite of tests that verify the correctness of the modules.
You should import the following dependency with test scope:

```xml
        <dependency>
            <groupId>io.debezium.quarkus</groupId>
            <artifactId>debezium-quarkus-testsuite-deployment</artifactId>
            <type>test-jar</type>
            <scope>test</scope>
        </dependency>
```
...
Please follow the guidelines present in the test suite [README.md](debezium-quarkus-testsuite-parent/README.md).
Furthermore, you have to create a `debezium-quarkus-testsuite.properties` file.
...
To achieve this, the Debezium Quarkus test suite contains an integration testing suite that verifies that the extension works in both single-engine and multi-engine configurations.
As with the deployment testing, please follow the guidelines present in the test suite [README.md](debezium-quarkus-testsuite-parent/README.md).