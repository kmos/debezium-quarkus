...
- **Use the Debezium Quarkus Engine extension** when you want lightweight, Kafka-free (optional) CDC *inside* your application with fast startup and small memory footprint (particularly with Quarkus Native/GraalVM). Best for POCs, dev environments, or simple streaming in microservices.
...
```bash
quarkus ext add io.debezium.quarkus:debezium-quarkus-postgres
```
